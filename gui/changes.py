...
def handle_queries():
    ...
    query_passengers_by_age = f"SELECT Train.Train_Number, Train.Train_Name, Train.Source_Station, Train.Destination_Station, " \
             ...
             f"Passenger.address,  Booked.Status " \
             f"FROM Passenger " \
             f"JOIN Booked ON Passenger.SSN = Booked.Passenger_ssn " \
             f"JOIN Train ON Train.Train_Number = Booked.Train_Number " \
             f"WHERE strftime('%Y', 'now') - (1900 + CAST(SUBSTR(bdate, 7) AS INTEGER)) BETWEEN {age_start} AND {age_end} "
...
def display_table_trains(result):
    ...
    tree['columns'] = tuple(['Train_Number', 'Train_Name', 'Source_Station', 'Destination_Station'])  # Replace with your column names
...
def display_table_age(result):
    ...
    tree = ttk.Treeview(table_window)
    tree['columns'] = tuple(['Train_Number', 'Train_Name', 'Source_Station', 'Destination_Station', 'Name','Address','Status'])  # Replace with your column names
